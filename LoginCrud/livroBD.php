...
if(isset($_POST['Submit'])) {	
	...
	if(empty($titulo) || empty($edicao) || empty($isbn)) {
		...
		$result = mysqli_query($mysqli, "INSERT INTO Livro (titulo, edicao, isbn) VALUES ('$titulo','$edicao','$isbn')");
		...
		if($result) {
			...
			echo "<a class='login-button' href='livroTable.php'>Ver Resultado</a>";
			echo "</div>";
		} else {
			// Exibir mensagem de erro se a inserção falhar
			echo "<font color='red'>Erro ao adicionar livro.</font>";
		}
	}
}
?>
