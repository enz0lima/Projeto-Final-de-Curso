...
if(isset($_POST['Submit'])) {	
	...
	if(empty($cargo) || empty($descricao) || empty($data_inicio) || empty($data_fim)) {
		...
		$result = mysqli_query($mysqli, "INSERT INTO Cargo (cargo, descricao, data_inicio, data_fim, cargo_ativo) VALUES ('$cargo','$descricao','$data_inicio','$data_fim','$cargo_ativo')");
		
		if($result) {
			// Exibir mensagem de sucesso se a inserção for bem-sucedida
			echo "<div class='success-box'>";
			echo "<div class='blur-effect'></div>";
			echo "<p class='success-message'>Cargo adicionado com sucesso!!</p>";
			echo "<a class='login-button' href='cargoTable.php'>Ver Resultado</a>";
			echo "</div>";
		} else {
